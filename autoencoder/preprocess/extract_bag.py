...
if __name__ == "__main__":
    ...
    blobs_to_process_later = []
    for topic, msg, time in messages:
        if topic == args.blob_topic:
	    data = [msg.x, msg.y]
	    blobs_to_process_later.append((data, time))
	else:
    ...
    for data, time in blobs_to_process_later:
	actual_hundredth_time = int(time.nsecs / args.factor)
	possible_times = groups[time.secs].keys()
	possible_times = sorted(possible_times, key=lambda x: abs(x - actual_hundredth_time))
	actual_hundredth_time = possible_times[0]
	groups[time.secs][actual_hundredth_time][args.blob_topic] += data
